...
@section('content')
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        ...
        <a href="{{ url('/admin/ppdb/create') }}" class="btn btn-primary btn-icon-split">
                        ...
                        <tbody>
                            @foreach ($ppdb as $item)
                                <tr>
                                    ...
                                    <td>{{ $item->judulPpdb }}</td>
                                    <td class="aksi-button text-center">
                                        <a href="{{ url('/admin/ppdb/' . Crypt::encryptString($item->idPpdb)) }}"
                                            class="btn btn-info btn-icon-split mt-1">
                                            <span class="icon text-white-50">
                                                <i class="fas fa-info-circle"></i>
                                            </span>
                                        </a>
                                        <a href="{{ url('admin/ppdb/' . Crypt::encryptString($item->idPpdb) . '/edit') }}"
                                        ...
                                        <form id="myForm" class="d-inline"
                                            action="{{ url('/admin/ppdb/' . Crypt::encryptString($item->idPpdb)) }}"
                                            method="POST">
                                            @method('DELETE')
                                            @csrf
