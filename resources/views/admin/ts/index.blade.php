...
@section('content')
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        ...
        <a href="{{ url('/admin/ts/create') }}" class="btn btn-primary btn-icon-split">
                            ...
                            @foreach ($ts as $item)
                                <tr>
                                    ...
                                    <td class="aksi-button text-center">
                                        <a href="{{ url('/admin/ts/' . Crypt::encryptString($item->idTs)) }}"
                                            class="btn btn-info btn-icon-split mt-1">
                                            <span class="icon text-white-50">
                                                <i class="fas fa-info-circle"></i>
                                            </span>
                                        </a>
                                        <a href="{{ url('admin/ts/' . Crypt::encryptString($item->idTs) . '/edit') }}"
                                        ...
                                        <form id="myForm" class="d-inline"
                                            action="{{ url('/admin/ts/' . Crypt::encryptString($item->idTs)) }}"
                                            method="POST">
                                            @method('DELETE')
                                            @csrf
