...
@section('content')
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        ...
        <a href="{{ url('/admin/ukk/create') }}" class="btn btn-primary btn-icon-split">
                            ...
                            @foreach ($ukk as $item)
                                <tr>
                                    ...
                                    <td class="aksi-button text-center">
                                        <a href="{{ url('/admin/ukk/' . Crypt::encryptString($item->idUkk)) }}"
                                            class="btn btn-info btn-icon-split mt-1">
                                            <span class="icon text-white-50">
                                                <i class="fas fa-info-circle"></i>
                                            </span>
                                        </a>
                                        <a href="{{ url('admin/ukk/' . Crypt::encryptString($item->idUkk) . '/edit') }}"
                                        ...
                                        <form id="myForm" class="d-inline"
                                            action="{{ url('/admin/ukk/' . Crypt::encryptString($item->idUkk)) }}"
                                            method="POST">
                                            @method('DELETE')
                                            @csrf
