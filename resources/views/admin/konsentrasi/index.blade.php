...
@section('content')
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        ...
        <a href="{{ url('/admin/konsentrasi/create') }}" class="btn btn-primary btn-icon-split">
                        ...
                        <tbody>
                            @foreach ($konsentrasi as $item)
                                <tr>
                                    ...
                                    <td>{{ $item->namaKonsentrasi }}</td>
                                    <td class="aksi-button text-center">
                                        <a href="{{ url('/admin/konsentrasi/' . Crypt::encryptString($item->idKonsentrasi)) }}"
                                            class="btn btn-info btn-icon-split mt-1">
                                            <span class="icon text-white-50">
                                                <i class="fas fa-info-circle"></i>
                                            </span>
                                        </a>
                                        <a href="{{ url('admin/konsentrasi/' . Crypt::encryptString($item->idKonsentrasi) . '/edit') }}"
                                        ...
                                        <form id="myForm" class="d-inline"
                                            action="{{ url('/admin/konsentrasi/' . Crypt::encryptString($item->idKonsentrasi)) }}"
                                            method="POST">
                                            @method('DELETE')
                                            @csrf
