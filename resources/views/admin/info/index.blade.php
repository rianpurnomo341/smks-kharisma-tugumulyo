...
@section('content')
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        ...
        <a href="{{ url('/admin/info/create') }}" class="btn btn-primary btn-icon-split">
                        ...
                        <tbody>
                            @foreach ($info as $item)
                                <tr>
                                    ...
                                    <td>{{ $item->judulInfo }}</td>
                                    <td class="aksi-button text-center">
                                        <a href="{{ url('/admin/info/' . Crypt::encryptString($item->idInfo)) }}"
                                            class="btn btn-info btn-icon-split mt-1">
                                            <span class="icon text-white-50">
                                                <i class="fas fa-info-circle"></i>
                                            </span>
                                        </a>
                                        <a href="{{ url('admin/info/' . Crypt::encryptString($item->idInfo) . '/edit') }}"
                                        ...
                                        <form id="myForm" class="d-inline"
                                            action="{{ url('/admin/info/' . Crypt::encryptString($item->idInfo)) }}"
                                            method="POST">
                                            @method('DELETE')
                                            @csrf
