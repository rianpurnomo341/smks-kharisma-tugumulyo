...
@section('content')
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        ...
        <a href="{{ url('/admin/bkk/create') }}" class="btn btn-primary btn-icon-split">
                            ...
                            @foreach ($bkk as $item)
                                <tr>
                                    ...
                                    <td class="aksi-button text-center">
                                        <a href="{{ url('/admin/bkk/' . Crypt::encryptString($item->idBkk)) }}"
                                            class="btn btn-info btn-icon-split mt-1">
                                            <span class="icon text-white-50">
                                                <i class="fas fa-info-circle"></i>
                                            </span>
                                        </a>
                                        <a href="{{ url('admin/bkk/' . Crypt::encryptString($item->idBkk) . '/edit') }}"
                                        ...
                                        <form id="myForm" class="d-inline"
                                            action="{{ url('/admin/bkk/' . Crypt::encryptString($item->idBkk)) }}"
                                            method="POST">
                                            @method('DELETE')
                                            @csrf
